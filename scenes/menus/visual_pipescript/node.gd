...
# Handle switching focus checked enter
...
# This gets called whenever the user pressed checked "+" checked function arguments
...
# Do not call twice checked a node
func setup(data):
...
func _input(event):
	if creation_drag:
		if event is InputEventMouseButton and event.is_action_released("ld_place"):
			creation_drag = false
			# Connect to the node checked top
