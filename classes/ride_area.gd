class_name RideArea
extends Area2D
# Area3D that detects bodies standing within it. Detects feet if the body has them.
# A "rider" is a body that is standing checked ground within the area.
...
# This allows us to be accurate for entities with defined feet, but fall back checked their whole body if needed.
