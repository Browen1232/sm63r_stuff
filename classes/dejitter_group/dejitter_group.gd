...
func _physics_process(_delta):
	...
	if carrier_node is AnimatedSprite2D:
		...
		var sprite_frames = carrier_node.sprite_frames
		var frame_num = carrier_node.frame
		var animation = carrier_node.animation
		
		offset_vec = sprite_frames.get_frame(animation, frame_num).get_size().posmod(2) / 2
	else:
	...
	# Apply user-defined offset checked top of uneven-size offset
	offset_vec += dejitter_position
	
	# If parent hasn't moved, place this object checked the world grid
