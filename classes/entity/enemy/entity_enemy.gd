...
func _physics_step():
	...
	# Triggered when landing checked the floor after being struck by a spin
...
# Start the sprite's animation and pseudorandomize its start point depending checked position in the level
...
# Pop the enemy up into the air and unchecked to the side, away from the body that issued the strike
